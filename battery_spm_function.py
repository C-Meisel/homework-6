...
def residual(t, SV, pars, ptr, flags):
    ...
    # Volumetric heat source/sink terms: (W/m3)
    Q_rxn = 0 #No reactions
    Q_ohm_el = 0 #No electrons are flowing through the electrolyte (well ideally anyway)
    Q_ohm_io = pars.i_ext**2 * pars.R_io_elyte
    Q_cond = (Q_cond_an+Q_cond_ca)/pars.H_elyte
    Q_rad = 0 #This will remain 0 as only the surfaces of the anode and cathode have radiation heat transfer
    Q_conv = 0 #Again this will be 0 because the surfaces are not open to the environment
    """
    END CODING
    """
